% DPS VG-AF, SIMULATION
function DPS_VGAF_sim = DPS_VGAF_simulation...
...
snr_relay_path = snrSRb.*snrRbDb./(snrSRb+snrRbDb+1);
% e2e SNR of the best relaying channel
snr_e2e = max(snrSDb,snr_relay_path);
% e2e SNR of the system (with selection combining)
%% COUNT EVENTS
for zz = 1:Sim_times
    if (snr_e2e(zz) < snrth)
        count = count + 1;
